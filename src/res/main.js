 ...
 * shows server status of all server elements using opacity
 * 
 function setServerStatus_obsolete() {
	let article, url;
	let nodes = document.getElementsByClassName("tim-server");
	for (let i = 0; i < nodes.length; i++) {
		let article = nodes[i];
		let url = article.attributes.getNamedItem("data-url").value
		checkOnline (url, function(u,s) {
			console.log('state of ', u, ' is ', s);
			if (s) article.classList.remove("w3-opacity-max");
			  else article.classList.add("w3-opacity-max");
			});
...
/***
 * check online status using fetch
 * 
function checkOnline(url, callback) {
	const timeout = 4000;
	const controller = new AbortController();
	const signal = controller.signal;
	const options = { method: 'HEAD', mode: 'no-cors', signal };
...
function checkServer() {
	...
	console.log('cs google.com', checkOnline('https://www.google.com', checkResult));
	console.log('cs hwn.de', checkOnline('https://www.hwn.de', checkResult));
	console.log('cs hwn.de/impressum', checkOnline('https://www.hwn.de/impressum', checkResult));
	console.log('cs hwn.name', checkOnline('https://www.hwn.name', checkResult));
	console.log('cs db-tim.netlify.app/bm/', checkOnline('https://db-tim.netlify.app/bm/', checkResult));
	console.log('cs db-tim.netlify.app/bm2.txt', checkOnline('https://db-tim.netlify.app/bm2.zxz', checkResult));
}
...
function checkUrl(url, timeout=1000) {
  ...
  return fetch(url, options)
    .then(setTimeout(() => { controller.abort() }, timeout))
    .then(response => console.log(url, 'Check server response:', response.statusText))
    .catch(error => console.error('url, Check server error:', error.message));
}
 *
 * ***/
